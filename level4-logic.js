// level4-logic.js
// Level 4: Player must place switch, LED, battery, and resistor
// all in their correct positions (like a series circuit).

import {Component, Property} from '@wonderlandengine/api';

export class Level4Logic extends Component {
    static TypeName = 'level4-logic';

    static Properties = {
        battery:  Property.object(),
        led:      Property.object(),
        resistor: Property.object(),
        sw:       Property.object(), // switch object

        batterySlot:  Property.object(),
        ledSlot:      Property.object(),
        resistorSlot: Property.object(),
        switchSlot:   Property.object(),

        positionTolerance: Property.float(0.1)
    };

    isCircuitBuilt() {
        console.log('Level4Logic.isCircuitBuilt() called');

        if (!this.battery || !this.led || !this.resistor || !this.sw ||
            !this.batterySlot || !this.ledSlot ||
            !this.resistorSlot || !this.switchSlot) {
            console.warn('Level4Logic: Missing references');
            return false;
        }
        ...
        const swOk = this._isNear(
            this.sw.getPositionWorld(),
            this.switchSlot.getPositionWorld(),
            this.positionTolerance
        );

        const ok = batOk && ledOk && resOk && swOk;

        // Optional: light LED green when everything is correct
        const meshMat =
        ...
        console.log('Level4Logic result:', ok);
        return ok;
    }
